...
import (
	...
	"sigs.k8s.io/kubebuilder/v3/pkg/plugins/operator/scaffolds"
)
...
func (p *createAPISubcommand) BindFlags(fs *pflag.FlagSet) {
	...
	// binding the image flag
	fs.StringVar(&p.image, "image", "", "Image name used for the operator")
...
func (p *createAPISubcommand) Scaffold(fs machinery.Filesystem) error {
	scaffolder := scaffolds.NewAPIScaffolder(p.config, *p.resource, p.force, p.image)
